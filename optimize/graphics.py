...
def plot_stepwiseconstant_waveform (control_waveform, nsteps, tstep, tstart, \
    fig=None, ax=None):
    ...
    ax.step(tstart + np.arange(nsteps+1)*tstep, \
            np.concatenate([control_waveform, [control_waveform[-1]]]), \
            where='post', lw=1.0)

    return fig, ax


def plot_pwc_controlwaveform(phi, Nsteps, Tstep, tstart, fig=None, ax=None, \
                             titlestring=None, outfilename=None):
    """
